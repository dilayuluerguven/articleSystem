...
export default function C_part() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const cCategory = data.find((cat) => cat.kod === "C");
        setCategories(cCategory ? [cCategory] : []);
      } catch (err) {
  ...
  const handleOk = async ({
    ...
    console.log("Bulunan path:", { ust_aktivite_id, alt_aktivite_id, aktivite_id });
    ...
    return cats.map((cat) => (
      <div key={cat.id} className="mb-4 ml-2">
        <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200 shadow-sm hover:shadow-md transition-all duration-300">
          ...
          {!isAdmin && !disallowedCodes.includes(cat.kod) && (
            <button
