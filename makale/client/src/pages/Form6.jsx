...
const Form6 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const apiUrl = formRecord?.id
        ? `http://localhost:5000/api/form6/${formRecord.id}/pdf`
        : "http://localhost:5000/api/form6/pdf";
      ...
      a.href = url;
      a.download = "FORM-6-PROFESORLUK-SOSYAL-MIMARLIK.pdf";
      a.click();
  ...
  const handleSave = async () => {
    ...
    try {
      const res = formRecord?.id
        ? await axios.put(
            `http://localhost:5000/api/form6/${formRecord.id}`,
            payload,
            { headers: { Authorization: `Bearer ${token}` } }
          )
        : await axios.post("http://localhost:5000/api/form6", payload, {
            headers: { Authorization: `Bearer ${token}` },
          });
  ...
  const autoFill = async () => {
    ...
    try {
      ...
      toast.info("Veriler sistemden başarıyla getirildi.");
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto px-6 pt-10">
        ...
        >
          ...
          <div className="bg-slate-800 text-white py-4 px-6 mb-10 text-center font-black rounded-2xl uppercase tracking-widest text-xs shadow-md">
            MİMARLIK / SOSYAL BİLİMLER PROFESÖRLÜK ASGARİ KOŞUL FORMU
          </div>
            ...
            <table className="w-full text-left border-collapse text-sm">
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                <MaddeRow
                  ...
                  text="1 tanesi Doçentlik sonrası olmak üzere başlıca eser niteliğinde en az 3 adet faaliyet yapmış olmalı."
                />
                <MaddeRow
                  label="b"
                  field={fields.b}
                  updateField={updateField}
                  text="Doçentlik sonrası çalışmalardan A-1a/b/g, A-2a/b, A-3a, C-1/2 türü en az 2 adet faaliyet yapmış olmalı."
                />
                <MaddeRow
                  label="c"
                  field={fields.c}
                  updateField={updateField}
                  text="Doçentlik sonrası; K-1/2/3, L ve M faaliyetlerinden en az 50 puan almış olmalı (veya A kategorisi telafisi)."
                />
                <MaddeRow
                  label="d"
                  field={fields.d}
                  updateField={updateField}
                  text="Doçentlik sonrası (A-G arası) + (K+L+M) faaliyetlerden en az 200 puan, toplamda 550 puan almış olmalı."
                />
                <MaddeRow
                  label="e"
                  field={fields.e}
                  updateField={updateField}
                  text="Başvurduğu bilim alanındaki tüm faaliyetlerden toplamda en az 650 puan almış olmalı."
                />
                <MaddeRow
                  label="f"
                  field={fields.f}
                  updateField={updateField}
                  text="Madde 2.6 koşulunu sağlamalı."
                />
                <MaddeRow
                  label="g"
                  field={fields.g}
                  updateField={updateField}
                  text="Madde 2.7 koşulunu sağlamalı."
                />
                <MaddeRow
                  label="h"
                  field={fields.h}
