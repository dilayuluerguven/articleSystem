  ...
  router.get("/:id/pdf", authMiddleware, async (req, res) => {
    try {
      ...
      const [sameCategoryRows] = await db.promise().query(
        ...
        `,
        [userId, data.ust_aktivite_id, data.alt_aktivite_id, data.aktivite_id]
      );

      let orderNumber;

      if (data.aktivite_kod?.startsWith("A-")) {
      ...
      const outputDocx = path.join(uploadDir, `form8_${data.id}.docx`);
      fs.writeFileSync(
        outputDocx,
      ...
      console.error("Form-8 PDF oluşturma hatası:", err);
